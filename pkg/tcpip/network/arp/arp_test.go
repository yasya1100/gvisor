...
func TestDirectRequest(t *testing.T) {
	...
	for i, address := range []tcpip.Address{stackAddr, remoteAddr} {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			requestsRecvKnownAddr := c.s.Stats().ARP.RequestsReceivedKnownTargetAddress.Value()
			...
			if got, want := c.s.Stats().ARP.RequestsReceivedKnownTargetAddress.Value(), requestsRecvKnownAddr+1; got != want {
				t.Errorf("got c.s.Stats().ARP.RequestsReceivedKnownTargetAddress.Value() = %d, want = %d", got, want)
			}
	...
	if got := c.s.Stats().ARP.RequestsReceivedUnknownTargetAddress.Value(); got != 1 {
		t.Errorf("got c.s.Stats().ARP.RequestsReceivedKnownTargetAddress.Value() = %d, want = 1", got)
	}
}
