...
type Container struct {
	...
	// profile is the profiling hook associatred with this container.
	profile *profile
}
...
// MakeNativeContainer sets up the struct for a DockerContainer using runc.
//
...
func (c *Container) CleanUp(ctx context.Context) {
	...
	// can cause Kill and Remove to hang. Note that profiling cleanups
	// are executed after the kill, below, because the kill itself will
	// call Stop on all the running profiles.
