...
func (c *Container) profileInit() {
	...
	// Assign a test name without the random bits of the container.
	testName := strings.Replace(c.logger.Name(), "/", "-", -1)
	c.profile = &profile{
		BasePath: filepath.Join(*pprofBaseDir, c.runtime, testName),
		Delay:    *pprofDelay,
...
func (p *profile) createProcess(c *Container) error {
	...
	args = append(args, fmt.Sprintf("--duration=%s", p.Duration))
...
// stopProcess forcefully stops the running process.
func (p *profile) stopProcess() error {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.done = true // Prevent create.
...
func (p *profile) Start(c *Container) error {
	if p.Delay != 0 {
		...
		time.AfterFunc(p.Delay, func() { p.createProcess(c) })
		return nil
	}
